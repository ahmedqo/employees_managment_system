...
<header class="w-full bg-blue-400 shadow-sm pb-8">
        ...
        <div class="w-max flex items-center gap-2 ml-auto">
            <div class="w-max relative">
                <button onclick="read(this)" x-toggle="#notification" x-property="pointer-events-none, opacity-0"
                    ...
                    @if ($notifications->where('isRead', 0)->count())
                        <span class="w-3 h-3 animate-pulse rounded-full bg-red-400 absolute top-2 right-2"></span>
                    @endif
                </button>
                <ul id="notification"
                    ...
                    <li
                        ...
                        <a href="{{ route('views.notifications.index') }}"
                            class="text-xs uppercase font-black w-full ml-auto text-blue-500 text-center hover:underline">
                            view all
                        </a>
                    </li>
                </ul>
            ...
            <div class="w-max relative">
                <button x-toggle="#dropdown" x-property="pointer-events-none, opacity-0"
                    class="flex items-center justify-center w-[42px] h-[42px] rounded-full focus:outline-1 focus:outline-1-2 outline-blue-400 bg-red-400">
                    <span
                ...
                <ul id="dropdown"
                    ...
                    <li class="w-full">
                        <a href="{{ route('views.profile.password') }}"
                            class="w-full flex gap-2 items-center px-4 py-2 outline-none text-gray-900 hover:bg-gray-900 hover:bg-opacity-10 focus:bg-gray-900 focus:bg-opacity-10 {{ Help::base('views.profile.password') }}">
                            <svg class="block w-4 h-4 pointer-events-none" fill="currentcolor" viewBox="0 0 48 48">
                                <path
                    ...
                    <li class="w-full">
                        <a href="{{ route('actions.profile.logout') }}"
                            class="w-full flex gap-2 items-center px-4 py-2 outline-none text-gray-900 hover:bg-gray-900 hover:bg-opacity-10 focus:bg-gray-900 focus:bg-opacity-10 {{ Help::base('actions.profile.logout') }}">
                            <svg class="block w-4 h-4 pointer-events-none" fill="currentcolor" viewBox="0 0 48 48">
                                <path

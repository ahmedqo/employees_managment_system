...
class ProjectController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Notification::create([
            ...
            'body' => 'you are assigned to manage project #' . $project->id . ' ' . $request->name,
        ]);
        ...
        if ($request->employees)
            foreach ($request->employees as $employee) {
                ...
                Notification::create([
                    'employee' => (int) $employee,
                    'body' => 'you are assigned to join the team on project #' . $project->id . ' ' . $request->name,
                ]);
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($project->manager != $request->manager) {
            Notification::create([
                'employee' => $request->manager,
                'body' => 'you are assigned to manage project #' . $id . ' ' . $request->name,
            ]);

            Notification::create([
                'employee' => $project->manager,
                'body' => 'you are no longer assigned to manage project #' . $id . ' ' . $project->name,
            ]);
        }
        ...
        if ($request->employees) {
            foreach ($employees as $employee) {
                if (!in_array($employee->employee, $request->employees))
                    Notification::create([
                        ...
                        'body' => 'you are no longer assigned to join the team on project #' . $id . ' ' . $request->name,
                    ]);
            }
            ...
            foreach ($request->employees as $employee) {
                ...
                if (!$employees->contains('employee', $employee)) {
                    Notification::create([
                        'employee' => (int) $employee,
                        'body' => 'you are assigned to join the team on project #' . $id . ' ' . $request->name,
                    ]);
                }
            }
        }
